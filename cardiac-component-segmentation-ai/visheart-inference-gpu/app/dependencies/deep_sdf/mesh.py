...
def create_mesh_4dsdf(
    ...
    while head < num_samples:
        ...
        sdf_pred = sdf_pred.squeeze(1).detach().cpu()
        samples[head: min(head + max_batch, num_samples), 3] = sdf_pred
        warped.append(new_xyz)
        head += max_batch
    ...
    # motion = (motion + 1) / 2 # Normalize values to [-1, 1]
...
def convert_sdf_samples_to_ply(
    ...
    ) # 14th Sept, james - Updated to marching_cubes for skimage v0.19.2

...
export function useLandmarkDetection(
  projectId: string,
  projectDimensions?: { width?: number; height?: number; frames?: number; slices?: number },
) {
  ...
  const expectedPredictionCount = Math.max(
    1,
    (projectDimensions?.frames ?? 0) > 1 && (projectDimensions?.slices ?? 0) > 0
      ? (projectDimensions?.frames ?? 1) * (projectDimensions?.slices ?? 1)
      : (projectDimensions?.slices ?? 0) > 0
      ? projectDimensions?.slices ?? 1
      : projectDimensions?.frames ?? 1,
  );
  ...
  const startPlaybackLoop = useCallback(() => {
    ...
    const tick = (now: number) => {
      ...
      const frameDuration = FRAME_DURATION_MS / Math.max(playbackSpeedRef.current, 0.25);
  ...
  useEffect(() => {
    ...
    if (landmarkApi.hasCached(projectId)) {
      ...
      landmarkApi.runDetectionByProject(projectId, undefined, false, expectedPredictionCount).then(applyResult).catch(() => {
  ...
  const handleRunDetection = useCallback(
    async (model = "unetresnet34-landmark") => {
      ...
      try {
        ...
        if (state.replacementFile) {
          ...
          result = await landmarkApi.runDetectionByProject(projectId, model, false, expectedPredictionCount);
        }
        ...
        if (process.env.NODE_ENV === "development" && !(err instanceof LandmarkApiError)) {
  ...
  const handleRerunDetection = useCallback(
    (model = "unetresnet34-landmark") => {
      ...
      setTimeout(async () => {
        ...
        setState((s) => ({ ...s, status: "running", error: null }));
        try {
          const result = await landmarkApi.runDetectionByProject(projectId, model, true, expectedPredictionCount);
          applyResult(result);
        } catch (err) {
          const msg =
            err instanceof LandmarkApiError
  ...
  const handlePlaybackSpeedChange = useCallback((speed: number) => {
    const safeSpeed = Math.max(0.25, Math.min(speed, 2));
    playbackSpeedRef.current = safeSpeed;
    lastTickRef.current = 0;
    setState((s) => ({ ...s, playbackSpeed: safeSpeed }));
  }, []);
